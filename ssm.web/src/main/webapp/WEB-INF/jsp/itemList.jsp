...
<body>
	...
	<table width="100%" border=1>
		<tr>
			<td><input type="button" name="all" id="checkboxAll" value="全 选">
		...
		<c:forEach items="${itemList }" var="item" varStatus="status">
			<tr>
				...
				<td><input type="text" name="itemsList[${status.index }].name"
					value="${item.name }" /></td>
				<td><input type="text" name="itemsList[${status.index }].pic"
					value="${item.pic }" /></td>
				...
				<td><a
					href="${pageContext.request.contextPath }/items/itemEdit/${item.id}">修改</a></td>
				<td><a
					href="${pageContext.request.contextPath }/items/delete/${item.id}">删除</a></td>
